...
def parse_template(filename,
    ...
    with codecs.open(os.path.join(path, filename), "rb", "utf-8") as f:
        text = f.read()
...
def render(content="hello world",
    ...
    if not content and not filename:
        raise SyntaxError("Must specify a filename or content")
    ...
    ), context=context, lexers=lexers, delimiters=delimiters, writer=writer))

    response = context["response"]  # required in exec(code)
    try:
